...
public class TestHeroesPage extends BaseTest {
	...
	/*
	 * Test validates that it is possible to add multiple inputs into hero table
	 * with different values from drop down menu option
	 * 
	...
	public Object[][] getDataFromDataprovider() {
		return new Object[][] { { "0Hero", "1", "Mesmer" }, { "1Hero", "1", "Necromancer" },
				{ "2Hero", "1", "Elementalist" }, { "3Hero", "1", "Ranger" }, { "4Hero", "1", "Thief" },
				{ "5Hero", "1", "Engineer" }, { "6Hero", "1", "Revenant" }, { "7Hero", "1", "Guardian" },
				{ "8Hero", "1", "Warrior" } };
	...
	/**
	 *
	 * Test validates that user can't create new hero, when hero level is beyond
	 * limit, by logging in as user, clicking on Heroes tab and on Add New Hero
	 * button, entering level beyond limit and checking if error message is visible.
	 *
	 * @author Jovan.Penic
	 */
	...
	public void testUserCanNotCreateNewHeroBeyondHeroLevel(String heroLevel) {
		...
		Assert.assertTrue(heroesPage.isHeroLevelErrorMessagePresent(),
				"User can add new hero with level beyond limit. ");
	...
	public Object[][] getDataFromNewHeroLevelBeyondLimitsDataProvider() {
		return new Object[][] { { "85" }, { "0" }, { "-10" }, };

	}

	/**
	 * Test validates that hero count for admin user is the same on Users page (list
	 * of users) and Hero Page(heroes list)
	 * 
	...
	/**
	 * Test check all the heroes from the Heroes page, and compares that list with
	 * the one took from My Heroes page
	 * 
	 * @author luka.ivancic
	 *
	 *
	...
	public void testUsersHeroesNumber() {
		...
		Assert.assertEquals(allUsersHeroes, myHeroesHeroes,
				"Number of all Users heroes is not the same as on the My Heroes page");
		log.info(
				"Checking the number of users heroes on the Heroes list, and the number of users heroes on the My Heroes page.");
	}
