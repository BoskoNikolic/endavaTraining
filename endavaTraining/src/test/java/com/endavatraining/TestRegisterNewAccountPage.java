...
public class TestRegisterNewAccountPage extends BaseTest {
	...
	/**
	 *
	 * Test validates that username field is NOT visible after entering wrong code
	 * with special characters in Registration Code text field. Then test validates
	 * if registration code error IS visible.
	 *
	 * @author Jovan.Penic
	 */
	@Test(priority = 0)
	public void testUserFieldVisibilityFalseRegCodeSpecChar() {
		...
		Assert.assertFalse(registerNewAccountPage.isUserNameFieldPresent(randomRegistrationCodeWithSpecCharacters),
				"Username field IS present after special character entry in registration code field. ");
		Assert.assertTrue(registerNewAccountPage.isRegistrationCodeErrorMessagePresent(),
				"Error message does NOT appear after special character entry in registration code field. ");
	 ...
	 *
	 * Test validates that username field is NOT visible after entering wrong code
	 * without special characters in Registration Code text field. Then test
	 * validates if registration code error IS visible.
	 *
	 * @author Jovan.Penic
	 */
	@Test(priority = 1)
	public void testUserFieldVisibilityFalseRegCode() {
		registerNewAccountPage = loginPage.openCreateAccount();
		Assert.assertFalse(registerNewAccountPage.isUserNameFieldPresent(randomRegistrationCodeWithLettersAndNumbers),
				"Username field IS present after random letters and numbers entry in registration code field. ");
		Assert.assertTrue(registerNewAccountPage.isRegistrationCodeErrorMessagePresent(),
				"Error message does NOT appear after random letters and numbers entry in registration code field. ");
		log.info("Tested visibility of user name field after entering wrong code without special characters");
	}

	/**
	 * The test opens the Create Account page, enters the correct credentials, and
	 * submits the form
	 * 
	 * @author: luka.ivancic
	 */
	...
	public void testCreateNewAccount() {
		registerNewAccountPage = loginPage.openCreateAccount();
		registerNewAccountPage.signUpNewUser(REGISTRATION_CODE, USER_NAME, FIRST_NAME, LAST_NAME, ABOUT,
				SECRET_QUESTION, SECRET_ANSWER, PASSWORD);
